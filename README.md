...
Set an autosave folder in the sidebar. LoreLock automatically loads and saves `lorelock_memory.json` and `provider_settings.json` in that folder. You can also open a memory JSON with the Streamlit upload control. The app only keeps the last selected autosave-folder path in `data/project_settings.json`.
